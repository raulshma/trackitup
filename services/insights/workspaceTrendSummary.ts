...
import { getReminderScheduleTimestamp, isReminderOpen } from "./workspaceInsights.ts";

export type WorkspaceTrendDestination = "visual-history" | "planner" | "inventory";
...
export function buildWorkspaceTrendSummary(
  ...
  const summaries = workspace.spaces.map<WorkspaceTrendSpaceSummary>((space) => {
    const logs = workspace.logs.filter((log) => log.spaceId === space.id);
    const currentLogs = logs.filter((log) => log.occurredAt.startsWith(monthKey));
    const previousLogs = logs.filter((log) => log.occurredAt.startsWith(previousMonthKey));
    const countPhotos = (items: typeof logs) =>
      items.reduce(
        (total, log) =>
          total +
          (log.attachments ?? []).filter((attachment) => attachment.mediaType === "photo")
            .length,
        0,
      );
    const countProofs = (items: typeof logs) =>
      items.reduce((total, log) => {
        ...
        const isProof = Boolean(log.routineId || log.reminderId || log.kind === "routine-run" || log.kind === "reminder");
    ...
    const metricAlerts = currentLogs
      ...
      .filter(
        (alert, index, alerts) =>
          alerts.findIndex((item) => item.metricId === alert.metricId) === index,
      )
      .slice(0, 3);
  ...
  const anomalies = summaries
    .flatMap<WorkspaceTrendAnomaly>((summary) => {
      ...
      if (summary.photoDelta <= -2 || (summary.previousPhotoCount >= 2 && summary.currentPhotoCount === 0)) {
        items.push({
  ...
  return {
    ...
    previousMonthKey,
    totals: {
      currentPhotoCount: summaries.reduce((total, item) => total + item.currentPhotoCount, 0),
      previousPhotoCount: summaries.reduce((total, item) => total + item.previousPhotoCount, 0),
      currentProofCount: summaries.reduce((total, item) => total + item.currentProofCount, 0),
      previousProofCount: summaries.reduce((total, item) => total + item.previousProofCount, 0),
    ...
    spaces: summaries
      .sort((left, right) => {
        const activityDelta =
          Math.abs(right.photoDelta) + Math.abs(right.proofDelta) - (Math.abs(left.photoDelta) + Math.abs(left.proofDelta));
